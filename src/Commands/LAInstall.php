 ...
 *
 * Command to install Crm package into project which moves lot of file from 'src/Installs' directory to Project
 */
class LAInstall extends Command
{
    ...
    protected $description = 'Install Crm Package. Generate whole structure for /admin.';
    ...
    /**
     * Generates and Moves files to install Crm package.
    ...
    {
        try {
            $this->info('Crm installation started...');
            
            $from = base_path('vendor/Zhovtyj/Crm/src/Installs');
            $to = base_path();
            ...
            if($this->confirm("Want to set your Database config in the .env file ?", true)) {
                ...
                $db_data['db'] = $this->ask('Database Name', 'Crm1');
            ...
            if($this->confirm("This process may change/append to the following of your existing project files:"
                ...
                if ($this->confirm('Would you like to customize this url ?', false)) {
                    ...
                    $laconfigfile =  $this->openFile($to."/config/Crm.php");
                    $arline = LAHelper::getLineWithString($to."/config/Crm.php", "'adminRoute' => 'admin',");
                    $laconfigfile = str_replace($arline, "    'adminRoute' => '" . $custom_admin_route . "',", $laconfigfile);
                    file_put_contents($to."/config/Crm.php", $laconfigfile);
                    config(['Crm.adminRoute' => $custom_admin_route]);
                }
                */
                ...
                // la-assets
                $this->line('Generating Crm Public Assets...');
                ...
                // $this->call('db:seed', ['--class' => 'CrmSeeder']);
                ...
                //if(!$this->fileContains($to."/app/Http/routes.php", "Crm.adminRoute")) {
                if(LAHelper::laravel_ver() >= 5.3 || LAHelper::laravel_ver() == 5.4) {
                ...
                $this->info("You can now login from yourdomain.com/" . config('Crm.adminRoute') . " !!!\n");
